...
try:
    ...
    print("RDKit required for comparison")
    sys.exit(1)
...
LOCAL_SMILES_DB = {
    ...
    "Estazolam": "ClC1=CC2=C(N3C(C(C4=CC=CC=C4)=NC2)=NN=C3)C=C1",
...
def canonicalize(smiles):
    ...
    if mol:
        # Remove stereo for basic connectivity check initially? 
        # No, keep stereo, NCI usually returns isomeric.
...
def fetch_nci_smiles(name):
    ...
    try:
        ...
        with urllib.request.urlopen(url, context=ctx, timeout=10) as response:
            ...
            # NCI sometimes returns multiple lines or fragments
...
def main():
    print(f"{'Drug':<25} | {'Source':<10} | {'Status'}")
    print("-" * 60)
    
    mismatches = 0
    
    for name, local_smiles in LOCAL_SMILES_DB.items():
        nci_smiles = fetch_nci_smiles(name)
        
        canon_local = canonicalize(local_smiles)
        
        if nci_smiles:
            canon_nci = canonicalize(nci_smiles)
            
            if canon_local == canon_nci:
                print(f"{name:<25} | NCI        | OK")
            else:
                print(f"{name:<25} | NCI        | MISMATCH / DIFFERENT ISOMER")
                # print(f"  Local: {canon_local}")
                # print(f"  NCI:   {canon_nci}")
                mismatches += 1
        else:
             print(f"{name:<25} | NCI        | NOT FOUND / ERROR")
        
        time.sleep(0.5)

    print(f"\nVerification Complete. Potential Mismatches: {mismatches}")
