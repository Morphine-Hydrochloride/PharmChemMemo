...
// Manual dictionary of SMILES for "Master" mode drugs in the app
// This bypasses the need for network fetching
const SMILES_DB = {
    // Chapter 5
...
async function patchData() {
    ...
    for (const item of data) {
        if (!item.smiles && SMILES_DB[item.en]) {
            item.smiles = SMILES_DB[item.en];
            updateCount++;
            console.log(`Patched: ${item.en}`);
        } else if (!item.smiles && item.en.includes("hydrochloride")) {
            // Try matching base name if hydrochloride variant
            const baseName = item.en.replace(" hydrochloride", "");
            if (SMILES_DB[baseName]) {
                item.smiles = SMILES_DB[baseName];
                updateCount++;
                console.log(`Patched (Base): ${item.en}`);
            }
        }
    }

    fs.writeFileSync(DATA_FILE, JSON.stringify(data, null, 2));
    console.log(`\nSuccess! Patched ${updateCount} items locally.`);
